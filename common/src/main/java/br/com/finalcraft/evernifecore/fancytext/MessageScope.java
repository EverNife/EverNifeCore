 ...
 * scope by definition - pass a {@link MessageContext} explicitly in that case.
 */
public final class MessageScope implements AutoCloseable {
    ...
    public static MessageScope open(@Nullable String label, @Nullable String subCommandName) {
        ...
        if (owner) {
            CURRENT.set(new MessageContext(label, subCommandName));
        }
        return new MessageScope(owner);
    }

    public static MessageContext currentOrEmpty() {
        MessageContext context = CURRENT.get();
        return context == null ? MessageContext.EMPTY : context;
    }
