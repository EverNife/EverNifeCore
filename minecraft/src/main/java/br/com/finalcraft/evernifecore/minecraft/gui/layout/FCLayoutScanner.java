...
public class FCLayoutScanner {
    ...
    public static <T extends LayoutBase> T loadLayout(ECPluginData ecPluginData, Config config, Class<T> layoutClass){
        ...
        for (Field declaredField : layoutInstance.getClass().getDeclaredFields()) {
            ...
            if (layoutIcon == null){
                ecPluginData.getLog().warning("[FCLayoutScanner] No LayoutIcon found on ["+ layoutClass.getClass().getName() + "] " + declaredField.toString());
            ...
            LocaleMessageImp localeMessage = new LocaleMessageImp(ecPluginData,
                    ICON_NAME,
                    false
            );;
            ...
            if (background && !itemSection.contains("")){
                ...
                try {
                    ...
                    ecPluginData.getLog().warning("[FCLayoutScanner] Failed to load LayoutIconBackground {" + ICON_NAME + "} from " + layoutClass.getSimpleName());
                    e.printStackTrace();
                    continue;
                }
            }
            ...
            try {
                if (itemSection.contains("Slot")){
                    Object slotObject = itemSection.getValue("Slot");
        ...
        SettingsScanner.loadSettings(ecPluginData, config, layoutInstance);
