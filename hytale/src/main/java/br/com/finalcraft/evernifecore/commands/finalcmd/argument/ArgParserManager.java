...
public class ArgParserManager {
    ...
    public static void addGlobalParser(Class clazz, Class<? extends ArgParser> parser){
        ...
        JavaPlugin plugin = FCJavaPluginUtil.getProvidingPlugin(parser);
        FCLocaleManager.loadLocale(plugin, true, parser);
    }

    public static void addPluginParser(JavaPlugin plugin, Class clazz, Class<? extends ArgParser> parser){
        ...
        JavaPlugin parserPlugin = FCJavaPluginUtil.getProvidingPlugin(parser); //Not always the same as the plugin adding it
        FCLocaleManager.loadLocale(parserPlugin, true, parser);
    }

    public static Class<? extends ArgParser> getParser(JavaPlugin plugin, Class argument){
