...
public abstract class SimpleThread {
    ...
    protected final JavaPlugin javaPlugin = FCJavaPluginUtil.getProvidingPlugin(this.getClass());
        ...
        if (thread == null || !thread.isAlive()){
            thread = new Thread(){
                ...
                public void run() {
                    try {
                        if (!isSilent()){
                            SimpleThread.this.javaPlugin.getLogger().atInfo().log("Starting Thread: " + this.getName());
                        }
                        if (executeOnStart){
                        ...
                        if (!silent){
                            SimpleThread.this.javaPlugin.getLogger().atInfo().log("Thread Shutdown: " + this.getName());
                        }
                    }
                }
            };
