...
public class FCLocaleScanner {
    ...
    public static List<LocaleMessageImp> scanForLocale(JavaPlugin plugin, boolean silent, Class localeClass){
        ...
        for (Field declaredField : localeClass.getDeclaredFields()) {
            if (declaredField.getType() == LocaleMessage.class || declaredField.getType() == LocaleMessageImp.class){
                ...
                if (!Modifier.isStatic(declaredField.getModifiers())){
                    plugin.getLogger().atSevere().log("[FCLocale] Found a LocaleMessage field that is not static! "  + getFieldAndClassName(declaredField));
                    continue;
                }
                ...
                if (!multiLocale && !simpleLocale){
                    plugin.getLogger().atSevere().log("[FCLocale] Found a LocaleMessage field that has no Annotations! " + getFieldAndClassName(declaredField));
                    continue;
                }
                ...
                try {
                    ...
                    plugin.getLogger().atSevere().log("[FCLocale] Failed to allow access to field! " + getFieldAndClassName(declaredField));
                    e.printStackTrace();
                    continue;
                }
                ...
                try {
                    ...
                    plugin.getLogger().atSevere().log("[FCLocale] Failed to create an instance of LocaleMessage.class at field! " + getFieldAndClassName(declaredField));
                    e.printStackTrace();
                    continue;
                }

                if (allKeys.contains(localeMessage.getKey().toLowerCase())){
                    plugin.getLogger().atWarning().log("[FCLocale] Found an already added {key==" + key + "} at field! Overriding last one! This is an Error! " + getFieldAndClassName(declaredField));
                }
        ...
        if (!atLeastOneLocaleField && !silent){
            plugin.getLogger().atWarning().log("[FCLocale] The class [" + localeClass.getName() + "] has no fields of LocaleMessage.class to be scanned!");
        }
    ...
    public static LocaleMessageImp scanForLocale(JavaPlugin plugin, String key, boolean shouldSyncToFile, FCLocaleData... locales){
        ...
        for (FCLocaleData fcLocale : locales) {
            ...
            if (newLocale.getFancyText(lang) != null){
                plugin.getLogger().atSevere().log("[FCLocale] Found a LocaleMessage with repeated {localeTypes} at key [" + key + "]! Ignoring new one!");
                continue;
            }
